"""Resource resolver contracts and deterministic implementations."""
...
from learning_compiler.agent.resources.catalog import resource_pool_for_corpus
...
def _resource_pool(request: ResourceRequest) -> tuple:
    ...
    corpus = f"{request.topic_spec.goal} {joined_scope} {context_focus} {request.node_title}"
    return resource_pool_for_corpus(corpus)
            ...
            resources.append(
                {
                    ...
                    "role": "definition",
                    **(
                        {"citation": fallback_item.citation}
                        if request.evidence_mode == "strict"
                        else {}
                    ),
                }
