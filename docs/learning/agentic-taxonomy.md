...
# D) Learning Roadmap (30/60/90 days mapped to taxonomy IDs)

This plan is **intentionally intense**. If you’re a true beginner, treat the first 2 weeks as “learn basic backend + Git + testing,” then continue.

## 30 days — Build a safe, observable single-agent system

**Goal:** ship a “useful but bounded” agent with tool calling, validation, and logs.

### Week 1: Foundations + minimal prototype

* Study/implement: **T1.1.1**, **T1.1.2**, **T1.2.1**
* Deliverable checkpoint:

  * A minimal agent loop with max steps and termination reasons logged
  * 3 tools with schemas and validators

### Week 2: Retrieval + reliability

* Study/implement: **T1.2.2**, **T1.2.3**, **T3.3.2** (basic RAG)
* Checkpoint:

  * RAG works on a small doc set with citations
  * Tool wrapper has timeouts + retries and a clear error taxonomy
...
* Study/implement: **T2.5.1**, **T1.3.2**, **T2.4.2**, **T3.2.3**
* Checkpoint:

  * Traces for each run (steps, tools, versions)
  * Basic policy enforcement (deny high-risk tools by default)
  * A small adversarial test set (prompt injection attempts)

**30-day milestone:** A demo that does something real *and* can explain what it did, with logs to prove it.

---

## 60 days — Make it testable, governable, and production-shaped

**Goal:** introduce CI evals, versioning, deployment basics, and human-in-the-loop.
...
## 90 days — Operate it like a real product (and survive incidents)

**Goal:** production observability, incident playbooks, security/compliance posture, cost optimization, and (optionally) multi-agent.
...
* Implement/process: **T4.1.4**, **T4.2.1**, **T1.3.2**
* Checkpoint:

  * At least 5 runbooks for common failure modes
  * One “game day” completed with postmortem + fixes
  * Continuous adversarial tests in CI
...
1. **Bounded Task Agent (single-agent)**
   ...
   * Includes tool calling, validators, repair, traces
   * Maps: **T1.1.1**, **T3.2.1**, **T2.4.1**, **T2.5.1**

2. **RAG Grounded Assistant with Freshness Controls**

   * Ingestion + retrieval eval + citations + index freshness monitoring
   * Maps: **T1.2.2**, **T3.3.2**, **T2.3.2**, **T4.1.3**

3. **Enterprise Automation Agent with Approvals + Audit**

   * Ticketing/CRM/email integration, approval gates for write actions, audit trails
   * Maps: **T3.2.4**, **T1.3.1**, **T2.3.3**, **T4.2.1**

4. **Reliability Stress Harness**

   * Tool failure injection, retries, idempotency, rollback/compensation
   * Maps: **T1.2.3**, **T3.2.2**, **T4.1.4**

5. **(Optional) Multi-Agent System**
...
That’s the boring scaffolding that lets the cool parts work without summoning production demons.
