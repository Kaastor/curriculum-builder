...
- `prompts/`: Prompt specifications for topic spec, mapping, validation, and workflow.
...
- `scripts/workflow.py`: Workflow CLI entrypoint (`init|status|next|validate|plan|iterate|run|archive|list`).
...
- `learning_compiler/workflow/cli.py`: Workflow parser and CLI dispatch.
- `learning_compiler/workflow/commands_basic.py`: Basic lifecycle commands (`init|status|next|list|archive`).
- `learning_compiler/workflow/commands_pipeline.py`: Pipeline commands (`validate|plan|iterate|run`).
- `learning_compiler/workflow/command_utils.py`: Shared command argument helpers.
- `learning_compiler/workflow/types.py`: Workflow enums and typed path contracts.
- `learning_compiler/workflow/fs.py`: Filesystem/env/run loading helpers.
- `learning_compiler/workflow/stage.py`: Stage inference/sync logic.
- `learning_compiler/workflow/exec.py`: External command/validator execution helpers.
- `learning_compiler/workflow/planning.py`: Deterministic topology, planning, and diff computations.
...
- Prefer package-first structure:
  ...
  - reusable logic belongs under `learning_compiler/` organized by domain (`workflow`, `validator`, etc.)
- Python **3.11** only.
